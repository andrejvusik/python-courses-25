...
Напишите декоратор @rate_limit, который разрешает только X вызовов в Y
секунд (например, 5 раз в 60 сек).
