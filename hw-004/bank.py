...
class Client:
    def __init__(self, login="", password="", fullname=""):
        self.login = login
        self.hash_password = get_hash(password)
...
class Bank:
    ...
    def __init__(self):
        ...
        if os.path.exists("db/clients.json"):
            with open("db/clients.json") as fh:
                self.clients_of_bank = json.loads(fh.read())
        else:
            os.system("mkdir db")
            self.clients_of_bank = dict()

        if os.path.exists("db/accounts.json"):
            with open("db/accounts.json") as fh:
                self.bank_accounts = json.loads(fh.read())
        else:
            os.system("mkdir db")
    ...
    def reg_client(self, client):
        ...
        self.clients_of_bank[self.client_id] = [
            client.login,
    ...
    def reg_account(self, account, balance, currency):
        self.bank_accounts[account.acc_id] = [
            account.client_id,
            account.currency,
            account.balance,
        ]
    ...
    def transfer_account(
        self, acc_number_1, acc_number_2, transfer_funds, currency_1, currency_2
    ):
        self.bank_accounts[acc_number_1][2] -= transfer_funds
        self.bank_accounts[acc_number_2][2] += (
            transfer_funds * self.CURRENCY_RATES[currency_1, currency_2]
        )
        with open("db/accounts.json", "w") as fh:
            json.dump(self.bank_accounts, fh, indent="    ")
...
def reg_new_client(bank_session):
    ...
    while True:
        ...
        if login in login_list:
            print(
                "A user with this login already exists, please think of another login."
            )
            continue
        else:
            break
    while True:
        ...
        if password == input("Please enter your password again: "):
            ...
            bank_session.client_id = bank_session.generate_id(client.__class__.__name__)
            # bank_session.Client = client
            bank_session.reg_client(client)
            os.system("cls" if os.name == "nt" else "clear")
            break
        else:
...
def show_bank_accounts(bank_session):
    client_accounts = bank_session.get_client_account(bank_session.client_id)
    ...
    all_in_usd = round(
        usd
        + (eur * Bank.CURRENCY_RATES["EUR", "USD"])
        + (byn * Bank.CURRENCY_RATES["BYN", "USD"]),
        2,
    )
    all_in_eur = round(
        (usd * Bank.CURRENCY_RATES["USD", "EUR"])
        + eur
        + (byn * Bank.CURRENCY_RATES["BYN", "EUR"]),
        2,
    )
    all_in_byn = round(
        (usd * Bank.CURRENCY_RATES["USD", "BYN"])
        + (eur * Bank.CURRENCY_RATES["EUR", "BYN"])
        + byn,
        2,
    )
    print(
    ...
    while True:
        ...
        if len(client_accounts) > 0:
            ...
            if user_select.upper() == "U":
                os.system("cls" if os.name == "nt" else "clear")
                top_up_acc_number, top_up_amount, currency_acc = top_up_account(
                    bank_session
                )
                print(
            ...
            if user_select.upper() == "W":
                os.system("cls" if os.name == "nt" else "clear")
                withdrawal_acc_number, withdrawal_funds, currency_acc = (
        ...
        if len(client_accounts) > 1:
            if user_select.upper() == "T":
                os.system("cls" if os.name == "nt" else "clear")
                acc_number_1, acc_number_2, transfer_funds, currency_acc = (
...
def acc_stat(bank_session):
    ...
    bank_session.print_account(acc_statement_filename, acc_stat_text)
...
def create_account(bank_session):
    ...
    while True:
        currency_acc = input(
            f"Whiten the account currency {possible_currencies}: "
        ).upper()
    ...
    if break_out_flag != True:
        ...
        client_acc.acc_id = bank_session.generate_id(client_acc.__class__.__name__)
        os.system("cls" if os.name == "nt" else "clear")
        bank_session.reg_account(client_acc, balance, currency)
    ...
    while True:
        ...
        if user_select.upper() == "Q":
            ...
            if bank_session.client_id == client_accounts[user_select][0]:
                bank_session.del_account(
                    user_select,
                    client_accounts[user_select][2],
                    client_accounts[user_select][1],
                )
                break
        else:
            os.system("cls" if os.name == "nt" else "clear")
    ...
    while True:
        ...
        if user_select.upper() == "Q":
            ...
            if bank_session.client_id == client_accounts[user_select][0]:
                ...
                while True:
                    top_up_amount = input(
                        f"Enter the amount to top up your account {currency}: "
                    )
                    if check_float(top_up_amount) and float(top_up_amount) >= 0:
                        ...
                        print("Please check the correctness of your choice.")
                        continue
                bank_session.top_up_account(user_select, top_up_amount, currency)
                break
        else:
            os.system("cls" if os.name == "nt" else "clear")
            print(f'You don\'t have such an account "{user_select}".')
            ...
            if bank_session.client_id == client_accounts[user_select][0]:
                ...
                while True:
                    withdrawal_amount = input(
                        f"Enter the amount to withdrawal from your account {currency}: "
                    )
                    if check_float(withdrawal_amount) and float(withdrawal_amount) >= 0:
                        ...
                        print("Please check the correctness of your choice.")
                        continue
                bank_session.withdrawal_account(
                    user_select, withdrawal_amount, currency
                )
                break
        else:
            os.system("cls" if os.name == "nt" else "clear")
            ...
            if bank_session.client_id == client_accounts[acc_number_1][0]:
                ...
                while True:
                    transfer_funds = input(
                        f"Enter the amount to transfer {currency_1}: "
                    )
                    if check_float(transfer_funds) and float(transfer_funds) >= 0:
                        ...
                        print("Please check the correctness of your choice.")
                        continue
                bank_session.transfer_account(
                    acc_number_1, acc_number_2, transfer_funds, currency_1, currency_2
                )
                break
        else:
            os.system("cls" if os.name == "nt" else "clear")
